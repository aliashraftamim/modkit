...
import { tammu_service } from "./tammu.service";

const createTammu = catchAsync(async (req: Request, res: Response) => {
  const result = await tammu_service.createTammu(req.body);
  sendResponse(res, {
...
const getAllTammu = catchAsync(async (req: Request, res: Response) => {
  const result = await tammu_service.getAllTammu(req.query);
  sendResponse(res, {
...
const getTammuById = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await tammu_service.getTammuById(id);
  if (!result) {
...
const updateTammu = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await tammu_service.updateTammu(id, updateData);
  if (!result) {
    return sendResponse(res, {
      statusCode: httpStatus.NOT_FOUND,
      success: false,
...
const softDeleteTammu = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await tammu_service.softDeleteTammu(id);
  if (!result) {
    return sendResponse(res, {
      statusCode: httpStatus.NOT_FOUND,
      success: false,
